  ...
  <div>
    ...
    <form id="sendAppealForm">
      <a href="javascript:void(0);" class="upload">
        选择文件 >
        <span class="unfile">未选择任何文件</span>
        <input
          class="replyFileid"
          name="appealFile"
          id="appealFile"
    ...
    <el-form label-position="right" label-width="80px" :model="userInfo">
      <el-form-item label="昵称：">
        <el-link>{{userInfo.name}}</el-link>
      </el-form-item>
      <el-form-item label="日志：">
        <router-link to="/log">{{userInfo.logInfo}}</router-link>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.$axios
      .post(this.GLOBAL.url_api + "/setUp", {
  ...
  methods: {
    pass() {
      if (
        document.getElementsByClassName("unfile")[0].innerHTML ==
        "未选择任何文件"
      ) {
        alert("亲，请上传文件");
      } else {
        ...
        if (this.isUpload != false) {
          $.ajax({
            url: this.GLOBAL.url_api + "/upload/Photo",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
