  ...
  <div id="singer-song">
    ...
    <div class="delete">
      <div v-for="(item, index) in audios" :key="index">
        <div
          class="slider"
          @click="music_btn(item.url,item.controlList,item.uploader,item.uploadTime)"
        >
          <span
            class="content"
            ...
            :style="deleteSlider"
          >
            <span :id="'Audio'+index">{{item.audioName}}</span>
            &nbsp;-&nbsp;{{item.uploader}}
            <!-- 插槽中放具体项目中需要内容         -->
            <slot></slot>
          </span>
          <div class="remove" ref="remove" @click.stop="delAudio(index)">删除</div>
        </div>
...
import { SearchMusic, SingerSong, DelAudio } from "./../apis/music.js";
...
export default {
  ...
  created() {
    this.$http
      .post(this.GLOBAL.url_api + "/music/singerSong?name=" + this.name)
      .then(function(data) {
        // console.log(data);
  ...
  methods: {
    ...
    async delAudio(index) {
      ...
      if (
        ...
        if (confirm("确认要删除该歌曲？")) {
          this.$store.commit(
            "delAudio",
            await DelAudio({ url: audioUrl.innerHTML })
          );

          // this.$axios
          //   .post(this.GLOBAL.url_api + "/music/delAudio", {
    ...
    async searchMusicName(e) {
      var SearchMusicList = await SearchMusic({
        url: this.search,
        name: this.name
      });
      ...
      this.audios = [];
      if (this.search === "") {
        for (var s = 0; s < SearchMusicList.length - 1; s++) {
          SearchMusicList[s].audioName = SearchMusicList[s].url;
          SearchMusicList[s].url =
            "./../nodeJs/audio/" + SearchMusicList[s].url;
          this.audios.push(SearchMusicList[s]);
        }
      } else {
        for (var s = 0; s < SearchMusicList.length; s++) {
          SearchMusicList[s].audioName = SearchMusicList[s].url;
          SearchMusicList[s].url =
            "./../nodeJs/audio/" + SearchMusicList[s].url;
          this.audios.push(SearchMusicList[s]);
      ...
      this.$store.commit("audiosPage", SearchMusicList);

      // this.$axios
      //   .post(this.GLOBAL.url_api + "/music/searchMusic", {
      //     url: this.search,
      //     name: this.name
      //   })
      //   .then(res => {
