...
<!--        <input type="text" id="search" v-model="search" v-on:input.prevent = "filteredBlogs()" required lay-verify="required" placeholder="请输入标题进行搜索" autocomplete="off" class="layui-input">  -->
        ...
        <div style="margin-top: 15px;">
          <el-input id="search" v-model="search" v-on:input.native = "filteredBlogs()" required lay-verify="required" placeholder="请输入标题进行搜索" autocomplete="off" class="input-with-select">
            <el-select v-model="select"  @change="selectCategory" slot="prepend" unselectable="on" placeholder="请选择">
...
<!--            <div class="icon-close" :class="{'iconClose':delDisplay}" v-on:click.prevent = "close(blog.id,index)">X</div>-->
            <i class="el-icon-circle-close" :class="{'iconClose':delDisplay}" v-on:click.prevent = "close(blog.id,index)"></i>
            <span class="NowTime"><i>{{blog.boketime}}</i></span>
        </div>
  ...
  methods:{
      ...
      if(this.pagenum > 0){
        this.pagenum--;
          ...
          if(this.pagenum>=0 && this.pagenumL > 0 && this.pagenumR > 5 ){
            ...
            this.$store.commit("currentPage",{pagenumL:this.pagenumL,pagenumR:this.pagenumR}); 
    ...
    bokenext(){
      ...
      // 我删除了，还是this.blogs.length还是以前的值
        if((this.pagenum*5) + 1 <= this.blogs.length || 
          (((this.pagenum-1)*5) + 1 <= this.blogs.length && this.blogs.length < (this.pagenum*5) + 1)){  
          this.pagenumL += 5;
          this.pagenumR = this.pagenumL + 5;
          this.$store.commit("currentPage",{pagenumL:this.pagenumL,pagenumR:this.pagenumR}); 
